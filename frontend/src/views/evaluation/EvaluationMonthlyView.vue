              ...
              <article v-for="component in detail.components" :key="component.code" class="evaluation-component-card" :class="{disabled:!component.enabled}">
                ...
                <template v-else-if="component.enabled&&component.code==='STATION'">
                  ...
                  <div v-for="row in component.breakdown||[]" :key="row.stationId" class="station-review-row">
                    ...
                    <p v-if="row.evaluations?.length" class="evaluation-muted">已评：{{row.evaluations.map((x:any)=>`${x.evaluatorName} ${scoreText(x.score)}分`).join('、')}}</p>
                ...
                <template v-else-if="component.enabled&&component.code==='TRAINING'">
                  <template v-if="editable(component)"><el-input-number v-model="inputFor('TRAINING').score" :min="0" :max="Number(component.fullScore||100)" :precision="2" controls-position="right"/><el-input v-model="inputFor('TRAINING').comment" type="textarea" :rows="3" maxlength="1000" show-word-limit placeholder="填写培训学习与综合表现"/><el-button type="primary" :disabled="!inputFor('TRAINING').comment.trim()" @click="submit(component)">保存培训评价</el-button></template>
                  <template v-else><p class="evaluation-muted">评价人：{{component.evaluatorName||'尚未提交'}}</p><p v-if="component.breakdown?.[0]?.comment" class="evaluation-muted">意见：{{component.breakdown[0].comment}}</p></template>
                </template>
