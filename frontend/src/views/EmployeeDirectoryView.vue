...
const auth=useAuthStore(),canMaster=computed(()=>auth.can('master:manage'));
...
onMounted(()=>{load();masters();if(canMaster.value)loadPending()});
</script>

<template>
  <div class="page">
    <div class="page-head"><h2>人员信息</h2><div><el-badge v-if="canMaster" :value="pendingCount" :hidden="!pendingCount"><el-button @click="reviewDialog=true">审核</el-button></el-badge><el-button type="primary" :loading="exporting" @click="exportRows" style="margin-left:8px">按当前筛选导出</el-button></div></div>
    ...
    <el-table :data="rows" stripe>
      ...
      <el-table-column label="服务站" width="130"><template #default="s"><el-button v-if="s.row.station_name&&canMaster" link type="primary" @click="showStationHistory(s.row)">{{s.row.station_name}}</el-button><span v-else>{{s.row.station_name||'-'}}</span></template></el-table-column>
