...
<template>
  <div class="page" v-loading="loading">
    ...
    <el-card v-if="canManage" class="section"><template #header>加扣分</template><div class="toolbar"><el-select v-model="adjustment.type" filterable><el-option label="加分" value="BONUS"/><el-option label="扣分" value="DEDUCTION"/></el-select><el-input-number v-model="adjustment.points" :min="0.01" :precision="2"/><el-input v-model="adjustment.reason" placeholder="原因"/><el-button :disabled="detail.locked||!adjustment.reason" @click="addAdjustment">登记</el-button></div></el-card>
    </template>
