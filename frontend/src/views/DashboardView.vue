...
const operationCards=computed(()=>[
  ...
  {key:'exam',title:'考试进度',description:'客观题自动评分，考试结束统一下发',icon:DocumentChecked,tone:'amber',rate:Number(operations.value.exam?.completion_rate||0),route:auth.can('exam:manage')?'/exams/plans':'/exams/my?section=results',stats:[['考试计划',operations.value.exam?.plan_total||0],['待批阅',operations.value.exam?.pending_review||0],['缺考',operations.value.exam?.absent_total||0]]},
