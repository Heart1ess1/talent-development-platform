...
const filters=reactive({keyword:'',courseId:null as number|null,sessionId:null as number|null,source:'',dateRange:[] as string[]})
...
async function load(){
  loading.value=true
  try{
    const [attendanceResponse,courseResponse,sessionResponse]=await Promise.all([
...
function reset(){
  Object.assign(filters,{keyword:'',courseId:null,sessionId:null,source:'',dateRange:[]})
  load()
}
  ...
  <div class="course-module-page">
    ...
    <el-dialog v-model="manualOpen" title="人工补录签到" width="min(540px, 92vw)" destroy-on-close>
      ...
      <el-form label-position="top">
        ...
        <el-form-item label="员工" required><el-select v-model="manual.employeeId" filterable placeholder="搜索员工"><el-option v-for="person in employees" :key="person.id" :label="`${person.name} · ${person.employeeNo||person.employee_no}`" :value="person.id"/></el-select></el-form-item>
