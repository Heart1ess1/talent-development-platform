...
import {filterTaskScoringEmployees,sortTaskScoringEmployees,taskScoringFilterOptions,type TaskScoreSortOrder,type TaskScoringEmployeeFilters} from './taskScoringFilters'
import {reviewerScopePayload,scopeMode,type ReviewerScopeDraft,type ReviewerScopeMode} from '@/types/taskReviewerScope'
...
const scoreSortOrder=ref<TaskScoreSortOrder>(null)
const filteredTaskEmployees=computed(()=>sortTaskScoringEmployees(filterTaskScoringEmployees(taskEmployees.value,employeeFilters),scoreSortOrder.value))
...
const hasEmployeeFilters=computed(()=>Boolean(employeeFilters.keyword.trim()||employeeFilters.batchId||employeeFilters.businessUnitId||employeeFilters.classId||employeeFilters.scoringStatus||scoreSortOrder.value))
...
function resetEmployeeFilters(){Object.assign(employeeFilters,{keyword:'',batchId:'',businessUnitId:'',classId:'',scoringStatus:''});scoreSortOrder.value=null}
function handleEmployeeSort({prop,order}:{prop?:string;order?:TaskScoreSortOrder}){if(prop==='final_score')scoreSortOrder.value=order||null}
function scopeDraft(row:any):ReviewerScopeDraft{return {id:Number(row.id),batchId:row.batch_id?Number(row.batch_id):null,businessUnitId:row.business_unit_id?Number(row.business_unit_id):null,classId:row.class_id?Number(row.class_id):null,reviewerIds:(row.reviewerIds||[]).map(Number),locked:Boolean(row.locked),label:row.label,coveredEmployees:Number(row.covered_employee_count||0)}}
async function openTask(row:any){if(selectedTask.value?.id!==row.id)resetEmployeeFilters();taskDrawer.value=true;detailLoading.value=true;try{selectedTask.value=(await api.get<any,Envelope<any>>(`/task-scoring/tasks/${row.id}`)).data;reviewerScopeDrafts.value=(selectedTask.value.reviewerScopes||[]).map(scopeDraft);reviewerScopeMode.value=scopeMode(reviewerScopeDrafts.value);reviewerScopePreview.value=undefined}finally{detailLoading.value=false}}
...
async function previewReviewerScopes(){reviewerScopePreview.value=(await api.post<any,Envelope<any>>(`/tasks/${selectedTask.value.id}/reviewer-scopes/preview`,{scopes:reviewerScopePayload(reviewerScopeDrafts.value)})).data}
async function saveReviewers(){
  savingReviewers.value=true
  try{await previewReviewerScopes();if(reviewerScopePreview.value&&!reviewerScopePreview.value.valid)return ElMessage.warning('请先处理未覆盖或重叠员工');await api.put(`/tasks/${selectedTask.value.id}/reviewer-scopes`,{scopes:reviewerScopePayload(reviewerScopeDrafts.value)});ElMessage.success('评分范围已更新');await Promise.all([openTask({id:selectedTask.value.id}),load()])}
  ...
  <div class="scoring-page">
    ...
    <section class="scoring-panel">
      ...
      <el-table v-loading="loading" :data="filteredTasks" empty-text="暂无评分任务">
        ...
        <el-table-column label="评分范围" min-width="210"><template #default="{row}"><span v-if="row.reviewer_scope_count">{{row.reviewer_scope_count}} 个评分范围 / {{row.reviewer_count}} 名评分人</span><span v-else>待分配</span><br v-if="row.reviewer_names"><small v-if="row.reviewer_names">{{row.reviewer_names}}</small></template></el-table-column>
    ...
    <el-drawer v-model="taskDrawer" size="min(1120px,96vw)" class="scoring-drawer">
      ...
      <div v-loading="detailLoading" v-if="selectedTask">
        ...
        <section class="reviewer-panel scope-config"><div class="scope-config-head"><div><h3>评分范围配置</h3><p>批次、板块、班级按交集匹配；开始评分后仅锁定对应范围。</p></div><el-button v-if="canManage" type="primary" :loading="savingReviewers" @click="saveReviewers">保存评分范围</el-button></div><TaskReviewerScopeEditor v-if="canManage" v-model:mode="reviewerScopeMode" v-model="reviewerScopeDrafts" :reviewer-options="reviewerOptions" :batches="employeeBatchOptions.map(item=>({id:item.id,name:item.label}))" :business-units="employeeBusinessUnitOptions.map(item=>({id:item.id,name:item.label}))" :class-options="employeeClassOptions" :preview="reviewerScopePreview" @preview="previewReviewerScopes"/><div v-else class="scope-summary"><article v-for="item in selectedTask.reviewerScopes" :key="item.id"><strong>{{item.label}}</strong><span>{{item.reviewers.map((reviewer:any)=>reviewer.display_name).join('、')}}</span><small>覆盖 {{item.covered_employee_count}} 人 · {{item.scope_status==='LOCKED'?'评分中/已锁定':item.scope_status==='COMPLETED'?'已完成':'未开始'}}</small></article><el-empty v-if="!selectedTask.reviewerScopes?.length" description="尚未分配评分范围"/></div></section>
        ...
        <el-table :data="filteredTaskEmployees" empty-text="未找到符合条件的员工" @sort-change="handleEmployeeSort">
          ...
          <el-table-column prop="final_score" label="最终平均分" width="125" sortable="custom" :sort-orders="['descending','ascending',null]"><template #default="{row}">{{row.final_score??'--'}}</template></el-table-column>
