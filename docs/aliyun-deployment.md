...
- 验证：前端 49 项、后端 149 项测试通过；CDN 生产构建及打包成功。
- 生产 JAR SHA-256：`8b4e3fa0b3126415499e8a6e23b954ec6ceb5078e599ad76711abc54193ed875`。
- 数据库备份：`/data/talent-platform/backups/mysql/talent-platform-20260911-160722.sql.gz`，SHA-256：`e572d95a2c06ac3199cc48f290e93fbc42381d94be626d29807e39b75377fe98`。
- 发布候选：`/data/talent-platform/releases/staging/cdn-7005d241-20260911`；81 个静态资源上传成功，CDN 主资源为 `https://static.yryhx.cn/assets/index-XrMy9rYI.js`。
- 生产结果：应用 UP，生产就绪检查 ready=true，Flyway V38 成功。员工 275 人、任务分配 129 条与发布前一致；2 个计划和 16 个题库保留为未分类，文件夹外键删除规则 SET NULL 已验证。
- 本次使用现有候选激活脚本，激活时出现两次短暂 502，自动重试后恢复。回退采用激活脚本保存的上一版 JAR/环境；V38 为新增表及可空字段，不主动回退数据库。
- 浏览器文件夹流程与布局已使用本地模拟数据验证；生产真实账号下的交互验收尚未执行。
