...
class EmployeeSpreadsheetTest {
  ...
  void importTemplateMatchesExportAndIncludesInstructions() throws Exception {
    ...
    try (var workbook = WorkbookFactory.create(
      ...
      assertThat(employeeSheet.getRow(1).getCell(8).getStringCellValue()).endsWith("X");
      ...
      assertThat(instructions.getRow(9).getCell(1).getStringCellValue()).isEqualTo("是");
      assertThat(instructions.getRow(9).getCell(2).getStringCellValue()).contains("大写X");
