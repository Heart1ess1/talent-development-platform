...
class PermissionServiceTest {
  ...
  @Test void fixedRolePermissionMatrix(){
    ...
    assertThat(service.permissions("MENTOR")).contains(Permissions.EMPLOYEE_READ,Permissions.EMPLOYEE_PORTRAIT_VIEW,Permissions.EVALUATION_SUBMIT,Permissions.TASK_SCORE).doesNotContain(Permissions.TASK_MANAGE,Permissions.TASK_REVIEW);
    assertThat(service.permissions("STATION_MANAGER")).contains(Permissions.EMPLOYEE_PORTRAIT_VIEW,Permissions.EVALUATION_SUBMIT,Permissions.TASK_SCORE).doesNotContain(Permissions.ATTENDANCE_MANAGE);
    assertThat(service.permissions("TRAINING_ADMIN")).contains(Permissions.EMPLOYEE_UPDATE,Permissions.COURSE_MANAGE,Permissions.TASK_MANAGE,Permissions.TASK_REVIEW,Permissions.TASK_SCORE,Permissions.EXAM_MANAGE).doesNotContain(Permissions.EMPLOYEE_WRITE,Permissions.USER_EMPLOYEE_MANAGE);
    assertThat(service.permissions("ADMIN")).contains(Permissions.EMPLOYEE_UPDATE,Permissions.EMPLOYEE_WRITE,Permissions.USER_EMPLOYEE_MANAGE,Permissions.USER_OPS_ROLE_MANAGE,Permissions.MASTER_MANAGE,Permissions.EVALUATION_VIEW,Permissions.TASK_SCORE).doesNotContain(Permissions.USER_ADMIN_MANAGE,Permissions.EVALUATION_SUBMIT);
    assertThat(service.permissions("SUPER_ADMIN")).contains(Permissions.USER_ADMIN_MANAGE,Permissions.AUDIT_READ,Permissions.TASK_SCORE);
  }
