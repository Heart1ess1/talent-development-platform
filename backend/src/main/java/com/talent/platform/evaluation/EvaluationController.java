...
public class EvaluationController {
  ...
  public ApiResponse<List<Map<String,Object>>> monthlyCandidates(@RequestParam YearMonth month,@RequestParam(required=false)String keyword,@RequestParam(required=false)String status){
    permissions.require(Permissions.EVALUATION_VIEW);var user=SecurityUtils.current();String sql="select e.id,e.employee_no,e.name,b.name batch_name,s.name station_name from employee e left join talent_batch b on b.id=e.batch_id left join service_station s on s.id=e.station_id where e.status='ACTIVE'";var args=new ArrayList<Object>();
    if("MENTORED".equals(user.dataScope())){sql+=" and (e.mentor_user_id=? or e.skill_mentor_user_id=?)";args.add(user.id());args.add(user.id());}
  ...
  private void submitManual(String component, Long employeeId, YearMonth month,Long scopeId, BigDecimal score, String comment) {
    ...
    if("STATION".equals(component)){if(scopeId==null||!service.stationApplies(employeeId,month,scopeId))throw new BusinessException(400,"请选择该员工当月实际所在站点");Integer managed=db.queryForObject("select count(*) from station_manager_scope where user_id=? and station_id=?",Integer.class,SecurityUtils.current().id(),scopeId);if(managed==null||managed==0)throw new BusinessException(403,"只能评价本人负责的站点");effectiveScope=scopeId;}else permissions.requireEmployee(employeeId);
    ...
    if("TRAINING".equals(component))db.update("delete from monthly_evaluation where employee_id=? and period_month=? and evaluator_type='TRAINING'",employeeId,month.atDay(1));
  ...
  private int pendingManualScores(YearMonth month){String role=SecurityUtils.current().role();if(!List.of("MENTOR","STATION_MANAGER","TRAINING_ADMIN").contains(role))return 0;return monthlyCandidates(month,null,"MY_PENDING").data().size();}
  ...
  private void requireEvaluationEmployee(Long employeeId,YearMonth month){if("STATION_MANAGER".equals(SecurityUtils.current().role())){if(!managesHistoricalStation(employeeId,month,SecurityUtils.current().id()))throw new BusinessException(403,"无权访问该员工当月站点评价");}else permissions.requireEmployee(employeeId);}
  @SuppressWarnings("unchecked") private boolean canCurrentUserEvaluatePending(Map<String,Object> component){if(!"PENDING".equals(component.get("status")))return false;String code=Objects.toString(component.get("code"),"");if("TRAINING".equals(code))return "TRAINING_ADMIN".equals(SecurityUtils.current().role());Object raw=component.get("breakdown");if(!(raw instanceof List<?> list))return false;return list.stream().filter(Map.class::isInstance).map(x->(Map<String,Object>)x).anyMatch(x->Boolean.TRUE.equals(x.get("canEvaluate"))&&"PENDING".equals(x.get("status")));}
