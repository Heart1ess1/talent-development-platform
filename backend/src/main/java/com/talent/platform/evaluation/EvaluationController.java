...
public class EvaluationController {
  ...
  public ApiResponse<List<Map<String,Object>>> monthlyCandidates(@RequestParam YearMonth month,@RequestParam(required=false)Long classId,@RequestParam(required=false)String keyword,@RequestParam(required=false)String status){
    permissions.require(Permissions.EVALUATION_VIEW);var user=SecurityUtils.current();String sql="select e.id,e.employee_no,e.name,e.class_id,b.name batch_name,cls.label class_name,s.name station_name from employee e left join talent_batch b on b.id=e.batch_id left join dictionary_item cls on cls.id=e.class_id and cls.type_code='CLASS' left join service_station s on s.id=e.station_id where e.status='ACTIVE'";var args=new ArrayList<Object>();
  ...
  private int pendingManualScores(YearMonth month){String role=SecurityUtils.current().role();if(!List.of("MENTOR","STATION_MANAGER","TRAINING_ADMIN").contains(role))return 0;return monthlyCandidates(month,null,null,"MY_PENDING").data().size();}
