...
public class EmployeeProfileController {
  ...
  private Map<String,Object> load(Long userId){var rows=db.queryForList("select e.id,e.employee_no,e.name,e.batch_id,e.business_unit_id,e.station_id,e.mentor_user_id,e.skill_mentor_user_id,e.school,e.major,e.education,e.birth_date,e.native_place,e.residence,e.phone,e.email,e.onboard_date,e.status,b.name batch_name,bu.name business_unit_name,s.name station_name,tm.display_name mentor_name,tm.display_name technical_mentor_name,sm.display_name skill_mentor_name from employee e left join talent_batch b on b.id=e.batch_id left join business_unit bu on bu.id=e.business_unit_id left join service_station s on s.id=e.station_id left join sys_user tm on tm.id=e.mentor_user_id left join sys_user sm on sm.id=e.skill_mentor_user_id where e.user_id=?",userId);if(rows.isEmpty())throw new BusinessException(404,"员工信息不存在");return rows.get(0);}
}
