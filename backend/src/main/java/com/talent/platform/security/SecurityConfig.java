...
public class SecurityConfig {
  ...
  @Bean SecurityFilterChain filter(HttpSecurity h,JwtFilter f,PasswordChangeFilter p,ObjectMapper om)throws Exception{return h.csrf(x->x.disable()).cors(x->{}).sessionManagement(x->x.sessionCreationPolicy(SessionCreationPolicy.STATELESS)).authorizeHttpRequests(x->x.requestMatchers("/api/v1/auth/login","/actuator/health","/","/index.html","/assets/**","/login","/dashboard","/employees","/employee-directory","/location-reports","/courses","/courses/**","/training-plans","/training-plans/**","/tasks","/task-scoring","/evaluation","/evaluation/**","/exams","/exams/**","/dictionaries","/users","/profile").permitAll().requestMatchers(HttpMethod.GET,"/api/v1/avatars/**").permitAll().requestMatchers(HttpMethod.OPTIONS,"/**").permitAll().anyRequest().authenticated()).exceptionHandling(x->x.authenticationEntryPoint((q,r,e)->{r.setStatus(401);r.setContentType("application/json;charset=UTF-8");om.writeValue(r.getWriter(),ApiResponse.error(401,"请先登录或登录已过期"));})).addFilterBefore(f,UsernamePasswordAuthenticationFilter.class).addFilterAfter(p,JwtFilter.class).build();}
}
