...
public class ExamController {
  ...
  @PostMapping("/questions") public ApiResponse<Long> question(@Valid @RequestBody QuestionRequest q)throws Exception{permissions.require(Permissions.EXAM_MANAGE);requireEnabledBank(q.bankId());validateQuestion(q.type(),q.options(),q.answer());db.update("insert into question_bank(bank_id,question_type,stem,options_json,answer_json,explanation,default_score,created_by,tags) values(?,?,?,?,?,?,?,?,?)",q.bankId(),q.type(),q.stem().trim(),json(q.options()),json(q.answer()),trim(q.explanation()),q.score(),SecurityUtils.current().id(),jsonTags(q.tags()));Long id=lastId();audit.log("CREATE_QUESTION","QUESTION",id,null,q);return ApiResponse.ok(id);}
  @PutMapping("/questions/{id}") public ApiResponse<Void> updateQuestion(@PathVariable Long id,@Valid @RequestBody QuestionRequest q)throws Exception{permissions.require(Permissions.EXAM_MANAGE);requireEnabledBank(q.bankId());validateQuestion(q.type(),q.options(),q.answer());if(db.update("update question_bank set bank_id=?,question_type=?,stem=?,options_json=?,answer_json=?,explanation=?,default_score=?,tags=? where id=?",q.bankId(),q.type(),q.stem().trim(),json(q.options()),json(q.answer()),trim(q.explanation()),q.score(),jsonTags(q.tags()),id)==0)throw new BusinessException(404,"题目不存在");audit.log("UPDATE_QUESTION","QUESTION",id,null,q);return ApiResponse.ok(null);}
  @GetMapping("/questions") public ApiResponse<List<Map<String,Object>>> questions(@RequestParam(required=false)Long bankId,@RequestParam(required=false)String type,@RequestParam(required=false)String keyword){permissions.require(Permissions.EXAM_MANAGE);StringBuilder sql=new StringBuilder("select q.id,q.bank_id,b.name bank_name,b.enabled bank_enabled,q.question_type,q.stem,q.options_json,q.answer_json,q.explanation,q.default_score,q.enabled,q.created_at,q.tags from question_bank q join exam_question_bank b on b.id=q.bank_id where q.question_type in ('SINGLE','MULTIPLE','TRUE_FALSE')");var args=new ArrayList<Object>();if(bankId!=null){sql.append(" and q.bank_id=?");args.add(bankId);}if(type!=null&&!type.isBlank()){sql.append(" and q.question_type=?");args.add(type);}if(keyword!=null&&!keyword.isBlank()){sql.append(" and q.stem like ?");args.add("%"+keyword.trim()+"%");}sql.append(" order by q.id desc");return ApiResponse.ok(db.queryForList(sql.toString(),args.toArray()));}
  ...
  @DeleteMapping("/questions/{id}") public ApiResponse<Void> deleteQuestion(@PathVariable Long id){permissions.require(Permissions.EXAM_MANAGE);Integer used=db.queryForObject("select (select count(*) from exam_paper_question where question_id=?)+(select count(*) from exam_attempt_question where question_id=?)",Integer.class,id,id);if(used!=null&&used>0)throw new BusinessException(400,"题目已被试卷或答卷使用，请停用后保留历史");if(db.update("delete from question_bank where id=?",id)==0)throw new BusinessException(404,"题目不存在");audit.log("DELETE_QUESTION","QUESTION",id,null,null);return ApiResponse.ok(null);}
  ...
  public void questionTemplate(HttpServletResponse response)throws Exception{
    ...
    try(var input=new ClassPathResource("templates/question-bank-template.xlsx").getInputStream()){
      input.transferTo(response.getOutputStream());
    }
  }
  @PostMapping("/questions/import") @Transactional public ApiResponse<ImportResult> importQuestions(@RequestParam Long bankId,@RequestParam MultipartFile file)throws Exception{permissions.require(Permissions.EXAM_MANAGE);requireEnabledBank(bankId);List<QuestionImportRow> rows=EasyExcel.read(file.getInputStream()).head(QuestionImportRow.class).sheet(0).doReadSync();if(rows.isEmpty())throw new BusinessException(400,"导入文件没有题目数据");var errors=new ArrayList<RowError>();var parsed=new ArrayList<QuestionRequest>();for(int i=0;i<rows.size();i++){try{parsed.add(parseImportRow(bankId,rows.get(i)));}catch(IllegalArgumentException e){errors.add(new RowError(i+2,"题目",e.getMessage()));}}if(!errors.isEmpty())return ApiResponse.ok(new ImportResult(0,errors));for(var q:parsed)db.update("insert into question_bank(bank_id,question_type,stem,options_json,answer_json,explanation,default_score,created_by,tags) values(?,?,?,?,?,?,?,?,?)",q.bankId(),q.type(),q.stem(),json(q.options()),json(q.answer()),q.explanation(),q.score(),SecurityUtils.current().id(),jsonTags(q.tags()));audit.log("IMPORT_QUESTIONS","QUESTION",null,null,Map.of("bankId",bankId,"count",parsed.size()));return ApiResponse.ok(new ImportResult(parsed.size(),List.of()));}
  @PostMapping("/papers")
  @Transactional
  public ApiResponse<Long> paper(@Valid @RequestBody PaperRequest q) throws Exception {
    ...
    if (q.dynamicAssembly()) {
      ...
      Long id = lastId();
      for (var rule : rules)
        db.update("insert into exam_paper_random_rule(paper_id,question_type,count,score,tags,bank_ids) values(?,?,?,?,?,?)",
                id, rule.type(), rule.count(), rule.score(), jsonTags(rule.tags()), jsonIds(rule.bankIds()));
  ...
  public ApiResponse<List<Map<String,Object>>> papers(){
    ...
    return ApiResponse.ok(db.queryForList("""
        select p.id,p.name,p.description,p.assembly_mode,p.dynamic_assembly,
               ...
               p.created_at,(select count(*) from exam_plan x where x.paper_id=p.id) plan_count,
  ...
  @GetMapping("/papers/{id}") public ApiResponse<Map<String,Object>> paperDetail(@PathVariable Long id){permissions.require(Permissions.EXAM_MANAGE);var rows=db.queryForList("select p.* from exam_paper p where p.id=?",id);if(rows.isEmpty())throw new BusinessException(404,"试卷不存在");var result=new LinkedHashMap<String,Object>(rows.get(0));boolean dynamic=truthy(result.get("dynamic_assembly"));if(dynamic)result.put("rules",db.queryForList("select question_type,count,score,tags,bank_ids from exam_paper_random_rule where paper_id=? order by id",id));else result.put("questions",db.queryForList("select q.id,q.stem,q.question_type,b.name bank_name,pq.score,pq.sort_order from exam_paper_question pq join question_bank q on q.id=pq.question_id join exam_question_bank b on b.id=q.bank_id where pq.paper_id=? order by pq.sort_order",id));return ApiResponse.ok(result);}
  @DeleteMapping("/papers/{id}") @Transactional public ApiResponse<Void> deletePaper(@PathVariable Long id){permissions.require(Permissions.EXAM_MANAGE);Integer used=db.queryForObject("select count(*) from exam_plan where paper_id=?",Integer.class,id);if(used!=null&&used>0)throw new BusinessException(400,"试卷已用于考试计划，不能删除，可保留作为历史版本");db.update("delete from exam_paper_random_rule where paper_id=?",id);db.update("delete from exam_paper_question where paper_id=?",id);if(db.update("delete from exam_paper where id=?",id)==0)throw new BusinessException(404,"试卷不存在");audit.log("DELETE_EXAM_PAPER","EXAM_PAPER",id,null,null);return ApiResponse.ok(null);}
  @PostMapping("/plans")
  @Transactional
  public ApiResponse<Long> plan(@Valid @RequestBody PlanRequest q){
    permissions.require(Permissions.EXAM_MANAGE);
    if(!q.endsAt().isAfter(q.startsAt()))throw new BusinessException(400,"考试结束时间必须晚于开始时间");
    if(Duration.between(q.startsAt(),q.endsAt()).toMinutes()<q.durationMinutes())throw new BusinessException(400,"考试开放时段不能短于考试时长");
    if(db.queryForObject("select count(*) from exam_paper where id=?",Integer.class,q.paperId())==0)throw new BusinessException(400,"所选试卷不存在");
    var ids=new LinkedHashSet<>(q.employeeIds());
    var batchIds=new LinkedHashSet<>(q.batchIds()==null?List.<Long>of():q.batchIds());
    var businessUnitIds=new LinkedHashSet<>(q.businessUnitIds()==null?List.<Long>of():q.businessUnitIds());
    validateEnabledIds("talent_batch",batchIds,"参考批次");
    validateEnabledIds("business_unit",businessUnitIds,"参考板块");
    String marks=String.join(",",Collections.nCopies(ids.size(),"?"));
    Integer active=db.queryForObject("select count(*) from employee where status='ACTIVE' and id in ("+marks+")",Integer.class,ids.toArray());
    ...
    for(Long eid:ids)db.update("insert into exam_assignment(plan_id,employee_id,assigned_by) values(?,?,?)",id,eid,SecurityUtils.current().id());
    audit.log("CREATE_EXAM_PLAN","EXAM_PLAN",id,null,Map.of("request",q,"scoreMonth",scoreMonth,"assignedCount",ids.size()));
    return ApiResponse.ok(id);
  }

  @GetMapping("/plans/candidates")
  public ApiResponse<List<Map<String,Object>>> planCandidates(@RequestParam(required=false)String batchIds,@RequestParam(required=false)String businessUnitIds,@RequestParam(required=false)Long classId,@RequestParam(required=false)Long classPositionId,@RequestParam(required=false)String keyword){
    permissions.require(Permissions.EXAM_MANAGE);
    var batches=parseIds(batchIds,"批次");
    var businessUnits=parseIds(businessUnitIds,"板块");
    StringBuilder sql=new StringBuilder("select e.id,e.employee_no,e.name,e.class_id,e.class_position_id,b.name batch_name,cls.label class_name,cp.label class_position_name,bu.name business_unit_name from employee e left join talent_batch b on b.id=e.batch_id left join dictionary_item cls on cls.id=e.class_id and cls.type_code='CLASS' left join dictionary_item cp on cp.id=e.class_position_id and cp.type_code='CLASS_POSITION' left join business_unit bu on bu.id=e.business_unit_id where e.status='ACTIVE'");
    var args=new ArrayList<Object>();
    if(!batches.isEmpty()){sql.append(" and e.batch_id in (").append(String.join(",",Collections.nCopies(batches.size(),"?"))).append(")");args.addAll(batches);}
    if(!businessUnits.isEmpty()){sql.append(" and e.business_unit_id in (").append(String.join(",",Collections.nCopies(businessUnits.size(),"?"))).append(")");args.addAll(businessUnits);}
    if(classId!=null){sql.append(" and e.class_id=?");args.add(classId);}
    if(classPositionId!=null){sql.append(" and e.class_position_id=?");args.add(classPositionId);}
    if(keyword!=null&&!keyword.isBlank()){sql.append(" and (e.name like ? or e.employee_no like ?)");String value="%"+keyword.trim()+"%";args.add(value);args.add(value);}
    sql.append(" order by b.name,bu.name,e.employee_no,e.id");
  ...
  public ApiResponse<Map<String,Object>> start(@PathVariable Long id,@Valid @RequestBody(required=false) StartAttemptRequest request) throws Exception {
    ...
    if (now.isBefore(from) || !now.isBefore(to)) throw new BusinessException(400, "不在考试开放时间");
  ...
  private Map<String,Object> attemptPayload(Long id,boolean includeQuestions){
    ...
    String sql = dynamic ? """
        select q.id,q.question_type,q.stem,q.options_json,aq.score,x.answer_json saved_answer,x.score answer_score,x.reviewer_comment
        from exam_attempt_question aq
        join question_bank q on q.id=aq.question_id
        left join exam_answer x on x.attempt_id=aq.attempt_id and x.question_id=q.id
        where aq.attempt_id=?
        """ : """
        select q.id,q.question_type,q.stem,q.options_json,pq.score,x.answer_json saved_answer,x.score answer_score,x.reviewer_comment
        from exam_attempt a
        join exam_plan p on p.id=a.plan_id
  ...
  private void assertOwnAttempt(Long id){var u=SecurityUtils.current();Integer n=db.queryForObject("select count(*) from exam_attempt a join employee e on e.id=a.employee_id where a.id=? and e.user_id=?",Integer.class,id,u.id());if(n==0)throw new BusinessException(403,"无权操作该考试");}
  ...
  private List<PaperQuestion> manualQuestions(List<PaperQuestion> questions){if(questions==null||questions.isEmpty())throw new BusinessException(400,"手动组卷至少选择一道题");var ids=new HashSet<Long>();for(var q:questions){if(!ids.add(q.questionId()))throw new BusinessException(400,"试卷中不能重复选择同一道题");Integer count=db.queryForObject("select count(*) from question_bank q join exam_question_bank b on b.id=q.bank_id and b.enabled=true where q.id=? and q.enabled=true and q.question_type in ('SINGLE','MULTIPLE','TRUE_FALSE')",Integer.class,q.questionId());if(count==0)throw new BusinessException(400,"自动评分试卷仅支持单选题、多选题和判断题；题目"+q.questionId()+"不可用");}return questions;}
  ...
  private List<RandomRule> validateRandomRules(List<RandomRule> rules) {
    ...
    for (var rule : rules) {
      if("SHORT".equals(rule.type()))throw new BusinessException(400,"自动评分试卷不支持简答题，请使用单选题、多选题或判断题");
      if(!QUESTION_TYPES.contains(rule.type())||!usedTypes.add(rule.type()))
        throw new BusinessException(400,"随机组卷题型配置无效或重复");
      if(rule.count()>0)selected.add(new RandomRule(rule.type(),rule.count(),rule.score(),normalizeTags(rule.tags()),normalizeIds(rule.bankIds())));
    }
  ...
  private List<Long> eligibleQuestionIds(RandomRule rule,String major,boolean includeCommon) throws Exception {
    StringBuilder sql = new StringBuilder(
            "select q.id from question_bank q join exam_question_bank b on b.id=q.bank_id and b.enabled=true where q.enabled=true and q.question_type=?");
    var args = new ArrayList<Object>();
    args.add(rule.type());
    var bankIds=normalizeIds(rule.bankIds());
    if(!bankIds.isEmpty()){
      sql.append(" and q.bank_id in (").append(String.join(",",Collections.nCopies(bankIds.size(),"?"))).append(")");
      args.addAll(bankIds);
    }
    var tags = normalizeTags(rule.tags());
    if (!tags.isEmpty()) {
      sql.append(includeCommon
              ? " and (q.tags is null or json_length(q.tags)=0 or json_overlaps(q.tags,cast(? as json)))"
              : " and q.tags is not null and json_overlaps(q.tags,cast(? as json))");
      args.add(jsonTags(tags));
    }
    if (major != null && !major.isBlank()) {
      sql.append(" and (q.tags is null or json_length(q.tags)=0 or json_contains(q.tags,?))");
      args.add(mapper.writeValueAsString(major.trim()));
    } else if (includeCommon) {
      sql.append(" and (q.tags is null or json_length(q.tags)=0)");
    }
  ...
  private void assembleDynamicAttempt(Long attemptId,Long paperId,Long employeeId) throws Exception {
    ...
    var rows = db.queryForList(
            "select question_type,count,score,tags,bank_ids from exam_paper_random_rule where paper_id=? order by id",paperId);
  ...
  private void validateQuestion(String type,JsonNode options,JsonNode answer){if(!QUESTION_TYPES.contains(type))throw new BusinessException(400,"仅支持单选题、多选题和判断题");if("TRUE_FALSE".equals(type)){if(!answer.isBoolean())throw new BusinessException(400,"判断题答案必须为正确或错误");return;}if(options==null||!options.isArray()||options.size()<2)throw new BusinessException(400,"选择题至少需要两个选项");var values=new HashSet<String>();options.forEach(x->values.add(x.asText()));if(values.size()!=options.size())throw new BusinessException(400,"题目选项不能重复");if("SINGLE".equals(type)){if(!answer.isTextual()||!values.contains(answer.asText()))throw new BusinessException(400,"单选题答案必须是已有选项");}else{if(!answer.isArray()||answer.isEmpty())throw new BusinessException(400,"多选题至少需要一个正确答案");for(JsonNode x:answer)if(!values.contains(x.asText()))throw new BusinessException(400,"多选题答案必须是已有选项");}}
  private QuestionRequest parseImportRow(Long bankId,QuestionImportRow row){
    ...
    if("TRUE_FALSE".equals(type)){
      ...
      for(String value:List.of(orEmpty(row.getOptionA()),orEmpty(row.getOptionB()),orEmpty(row.getOptionC()),orEmpty(row.getOptionD()),orEmpty(row.getOptionE()),orEmpty(row.getOptionF())))if(!value.isBlank())values.add(value.trim());
      ...
      for(String letter:raw.toUpperCase().split("[,，、;；\\s]+")){
        if(letter.length()!=1||letter.charAt(0)<'A'||letter.charAt(0)>='A'+values.size())throw new IllegalArgumentException("正确答案请填写有效选项字母，如A或A,C");
    ...
    return new QuestionRequest(bankId,type,stem,options,answer,trim(row.getExplanation()),row.getScore(),parseTags(row.getTags()));
  }
  ...
  private List<Long> normalizeIds(List<Long> ids){return ids==null?List.of():ids.stream().filter(Objects::nonNull).distinct().toList();}
  private List<Long> parseJsonIds(Object raw) throws Exception {
    if(raw==null)return List.of();
    JsonNode node=mapper.readTree(String.valueOf(raw));
    if(!node.isArray())return List.of();
    var ids=new ArrayList<Long>();
    node.forEach(x->{if(x.canConvertToLong())ids.add(x.asLong());});
    return normalizeIds(ids);
  }
  private String jsonIds(List<Long> ids) throws Exception {
  ...
  private Long lastId(){return db.queryForObject("select last_insert_id()",Long.class);}private LocalDateTime time(Map<String,Object> m,String key){Object raw=m.get(key);if(raw instanceof LocalDateTime value)return value;if(raw instanceof Timestamp value)return value.toLocalDateTime();if(raw instanceof java.sql.Date value)return value.toLocalDate().atStartOfDay();if(raw instanceof java.util.Date value)return LocalDateTime.ofInstant(value.toInstant(),ZoneId.systemDefault());if(raw instanceof CharSequence value){String text=value.toString().trim();if(text.isEmpty())throw new BusinessException(500,"考试时间数据异常");try{return Timestamp.valueOf(text).toLocalDateTime();}catch(IllegalArgumentException ignored){return LocalDateTime.parse(text.replace(' ','T'));}}throw new BusinessException(500,"考试时间数据异常");}private String json(JsonNode n)throws Exception{return n==null?null:mapper.writeValueAsString(n);}private JsonNode tree(Object x)throws Exception{return x==null?null:mapper.readTree(String.valueOf(x));}
}
