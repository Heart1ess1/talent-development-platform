...
public class ExamController {
  ...
  public ApiResponse<List<Map<String,Object>>> planCandidates(@RequestParam(required=false)String batchIds,@RequestParam(required=false)String businessUnitIds,@RequestParam(required=false)String keyword){
    permissions.require(Permissions.EXAM_MANAGE);
    var batches=parseIds(batchIds,"批次");
    var businessUnits=parseIds(businessUnitIds,"板块");
    StringBuilder sql=new StringBuilder("select e.id,e.employee_no,e.name,b.name batch_name,bu.name business_unit_name from employee e left join talent_batch b on b.id=e.batch_id left join business_unit bu on bu.id=e.business_unit_id where e.status='ACTIVE'");
