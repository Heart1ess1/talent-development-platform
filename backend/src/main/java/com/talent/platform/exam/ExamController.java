...
public class ExamController {
  ...
  public ApiResponse<List<Map<String,Object>>> planCandidates(@RequestParam(required=false)String batchIds,@RequestParam(required=false)String businessUnitIds,@RequestParam(required=false)Long classId,@RequestParam(required=false)String keyword){
    permissions.require(Permissions.EXAM_MANAGE);
    var batches=parseIds(batchIds,"批次");
    var businessUnits=parseIds(businessUnitIds,"板块");
    StringBuilder sql=new StringBuilder("select e.id,e.employee_no,e.name,e.class_id,b.name batch_name,cls.label class_name,bu.name business_unit_name from employee e left join talent_batch b on b.id=e.batch_id left join dictionary_item cls on cls.id=e.class_id and cls.type_code='CLASS' left join business_unit bu on bu.id=e.business_unit_id where e.status='ACTIVE'");
